...
SUCK_DELAY_MS = 0     # รอหัวดูดจับบล็อก
RELEASE_DELAY_MS = 0  # รอหัวดูดปล่อยบล็อก
#300 100 50  0
#350 150 100 0
...
class Mover:
    ...
    แล้วรอแค่บล็อกก่อนหน้า (คิวมีไม่เกิน 2 บล็อก ~22 คำสั่ง < 32)"""
    ...
    def pick_and_place(self, src, tgt, safe_z):
        # ไม่ยกขึ้นหลังวาง: การยกครั้งถัดไป (lift) จะยกตรงไปที่ Safe Z ของบล็อกถัดไปในครั้งเดียว
        self.lift(safe_z)
        self.move(src["x"], src["y"], safe_z, src["r"])
        self.move(src["x"], src["y"], src["z"], src["r"])
        self.suck(True, SUCK_DELAY_MS)
        self.move(src["x"], src["y"], safe_z, src["r"])
        self.move(tgt["x"], tgt["y"], safe_z, tgt["r"])
...
def safe_z_for(base, block_h, tower_height):
    if tower_height <= 1:
        return base + 2.5 * block_h
    if tower_height == 2:
        return base + 3.5 * block_h
    return base + 4.5 * block_h
...
def run_operation(device, settings):
    ...
    phase1_z = safe_z_for(base, block_h, 0)
    for i in range(1, 5):
        pick, temp = positions.get(f"pick_{i}"), positions.get(f"temp_{i}")
        if pick and temp:
            print(f"📦 บล็อก {i}: pick -> temp")
            mover.pick_and_place(at(pick, table_z(pick)), at(temp, table_z(temp)), phase1_z)
    ...
    for i in range(1, 5):
        ...
        layer += 1
    mover.lift(safe_z_for(base, block_h, layer))
    mover.finish()
